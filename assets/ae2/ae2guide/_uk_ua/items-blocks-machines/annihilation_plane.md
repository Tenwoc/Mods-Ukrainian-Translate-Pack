...
Площина руйнування розіб’є блок або підбере предмет лише тоді, коли зможе зберегти результат у своїй мережі. Це означає, що для фільтрації _ви повинні обмежити те, що може зберігатися в його мережі_, найімовірніше, розмістивши його в [підмережі](../ae2-mechanics/subnetworks.md). <ItemLink id="storage_bus" /> або [комірка](../items-blocks-machines/storage_cells.md) може бути [розгалужена/відфільтрована](cell_workbench.md) для досягнення цього.
...
Знову ж таки, фільтр площини працює за _дропом предметів_, тому, наприклад, якщо ви хочете отримувати <ItemLink id="minecraft:amethyst_cluster" />, вам потрібна додаткова площина, зачарована Шовковим дотиком, інакше вона розбиватиме кожну попередню стадію зародка, оскільки технічно здатна зберегти «нічого», яке випадає з них.
